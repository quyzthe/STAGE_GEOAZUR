...
from myODM import camera
from myODM import location
from myODM.photo import find_largest_photo_dims, find_largest_photo
from myODM.photo import ODM_Photo
from myOSfM.large import metadataset
from myOSfM.large import tools
from myOSfM.actions import undistort
from myOSfM.dataset import DataSet
from myOSfM.types import Reconstruction
from myOSfM import report
from myODM.multispectral import get_photos_by_band
from myODM.gpu import has_popsift_and_can_handle_texsize, has_gpu
from myOSfM import multiview, exif
from myOSfM.actions.export_geocoords import _transform
...
class OSFMContext:
    ...
    def run(self, command):
        ...
        print(f"Open_sfm_bin directory: {osfm_bin}")
        print(f"Command: {command}")
        print(f"Project path: {self.opensfm_project_path}")
        print(f"================== Running command: {command} ==================")
        system.run('"%s" %s "%s"' %
                    (osfm_bin, command, self.opensfm_project_path))
        print(f"================== Run command {command} successfully ==================")
    ...
    def photos_to_metadata(self, photos, rolling_shutter, rolling_shutter_readout, rerun=False):
        print('I am the first line of the photo to metadata function')

        metadata_dir = self.path("exif")

        print('Directory metadata: ' + metadata_dir)
        
        print(rerun)

        print(f"Exif files exist or not: {io.dir_exists(metadata_dir)}")
        ...
        print('I now start to run make directory to create those exif files')
        os.makedirs(metadata_dir, exist_ok=True)
        print('I can reach where we make the directory of those exif files')

        print(f'Exif files exist or not: {io.dir_exists(metadata_dir)}')
        camera_models = {}
        data = DataSet(self.opensfm_project_path)

        print("I am checking what is the photos")
        print("Number of photos:", len(photos))
        print("Photos content:", photos)
        ...
        if data.camera_models_overrides_exists():
            print("Camera model overrides already existed")
    ...
    #     features_dir = self.path("features")
    #     print('I am in the feature matching function')
    ...
    def feature_matching(self, rerun=False):
        ...
        print(f"[DEBUG] rerun = {rerun}")
        print(f"[DEBUG] features_dir = {features_dir}")
        print(f"[DEBUG] dir_exists = {io.dir_exists(features_dir)}")
        print(f"[DEBUG] has gpu_sift = {hasattr(self, 'gpu_sift_feature_extraction')}")
        
        print('I am in the feature matching function')
        
        if not io.dir_exists(features_dir) or rerun:
            print("[DEBUG] Entering detect_features block")
            try:
                print("[DEBUG] Running detect_features...")
                self.run('detect_features')
                print("[DEBUG] detect_features completed successfully")
            except system.SubprocessException as e:
                print(f"[DEBUG] detect_features failed with error: {e}")
                if hasattr(self, 'gpu_sift_feature_extraction'):
                    print("[DEBUG] Attempting fallback to CPU SIFT...")
                    log.ODM_WARNING("GPU SIFT extraction failed, maybe the graphics card is not supported? Attempting fallback to CPU")
                    self.update_config({'feature_type': "SIFT"})
                    print(f"[DEBUG] features_dir exists before rmtree: {os.path.exists(features_dir)}")
                    if os.path.exists(features_dir):
                        shutil.rmtree(features_dir)
                        print("[DEBUG] features_dir removed")
                    print("[DEBUG] Running detect_features with CPU...")
                    self.run('detect_features')
                    print("[DEBUG] CPU detect_features completed successfully")
                else:
                    print("[DEBUG] No GPU fallback available, re-raising exception")
                    raise e
        else:
            print(f"[DEBUG] Skipping detect_features, already done")
            log.ODM_WARNING('Detect features already done: %s exists' % features_dir)

        print("[DEBUG] Calling match_features...")
        self.match_features(rerun)
        print("[DEBUG] match_features completed")
