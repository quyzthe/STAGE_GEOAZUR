...
def run(cmd, env_paths=[context.superbuild_bin_path], env_vars={}, packages_paths=context.python_packages_paths, quiet=False):
    ...
    print(f"[DEBUG] env_paths     = {env_paths}")
    print(f"[DEBUG] env_vars      = {env_vars}")
    print(f"[DEBUG] packages_paths= {packages_paths}")
    print(f"[DEBUG] quiet         = {quiet}")
    ...
    if sys.platform == 'darwin':
        cmd = "export DYLD_LIBRARY_PATH=\"%s\" && %s" % (env.get("DYLD_LIBRARY_PATH", ""), cmd)
        print(f"[DEBUG] cmd after DYLD patch = {cmd}")

    for k in env_vars:
        env[k] = str(env_vars[k])
        print(f"[DEBUG] env_var set: {k} = {env_vars[k]}")
