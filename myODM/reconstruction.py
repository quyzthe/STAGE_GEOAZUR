...
from myOSfM.align import align_reconstruction, apply_similarity
from myOSfM.context import current_memory_usage, parallel_map
from myOSfM.dataset_base import DataSetBase
...
def log_bundle_stats(bundle_type: str, bundle_report: Dict[str, Any]) -> None:
    if bundle_report is None:
        logger.debug(f"Bundle {bundle_type}: no report available (bundle returned None)")
        return
    
    times = bundle_report.get("wall_times", {}) or {}
    time_secs = times.get("run", 0) + times.get("setup", 0) + times.get("teardown", 0)
    num_images = bundle_report.get("num_images", 0)
    num_points = bundle_report.get("num_points", 0)
    num_reprojections = bundle_report.get("num_reprojections", 0)
...
def bundle(
    ...
    if report is None:
        report = {
            "wall_times": {"run": 0.0, "setup": 0.0, "teardown": 0.0},
            "num_images": len(reconstruction.shots),
            "num_points": len(reconstruction.points),
            "num_reprojections": 0,
            "brief_report": "Bundle adjustment skipped (stub implementation)",
        }
    
    log_bundle_stats("GLOBAL", report)
    logger.debug(report.get("brief_report", ""))
    return report
