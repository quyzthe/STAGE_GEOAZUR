...
from myOSfM.stub_files import pygeometry, pymap
from myOSfM.geo import TopocentricConverter
...
class Reconstruction:
    ...
    def _setup_from_map(self, map_obj):
        self.map = map_obj
        self.camera_view = pymap.CameraView(self.map._cameras)
        self.bias_view = pymap.BiasView(self.map._biases)
        self.rig_cameras_view = pymap.RigCameraView(self.map._rig_cameras)
        self.rig_instances_view = pymap.RigInstanceView(self.map._rig_instances)
        self.shot_view = pymap.ShotView(self.map._shots)
        self.pano_shot_view = pymap.PanoShotView(self.map._pano_shots)
        self.landmark_view = pymap.LandmarkView(self.map._landmarks)
    ...
    def create_shot(
        ...
        try:
            _rig_cameras_keys = list(self.rig_cameras)
        except Exception:
            _rig_cameras_keys = []
        
        if (not rig_camera_id) and (camera_id not in _rig_cameras_keys):
    ...
    def add_observation(
        self, shot_id, lm_id, observation
    ):
